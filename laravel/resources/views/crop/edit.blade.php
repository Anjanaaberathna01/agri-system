...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Poppins',sans-serif; }
        ...
        .header { background:linear-gradient(135deg,#4a7c2c 0%,#6ba545 100%); color:#fff; padding:2rem; border-radius:12px; margin-bottom:1.5rem; }
        ...
        .current-image { margin-top:0.5rem; max-width:180px; border-radius:8px; }
    </style>
</head>
<body>
    ...
    <div class="container">
        <div class="header">
            <h1><i class="fas fa-pen"></i> Edit Crop</h1>
            <p style="opacity:0.9;">Update details for {{ $crop->name }}</p>
        </div>
        ...
        <div class="card">
            ...
            <form action="{{ route('admin.crops.update', $crop) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $crop->name) }}" required>
                </div>

                <div class="form-group">
                    <label for="type">Type <span class="required">*</span></label>
                    <input type="text" id="type" name="type" value="{{ old('type', $crop->type) }}" required>
                </div>

                <div class="form-group">
                    <label for="price">Price (Rs) <span class="required">*</span></label>
                    <input type="number" step="0.01" min="0" id="price" name="price" value="{{ old('price', $crop->price) }}" required>
                </div>

                <div class="form-group" style="display:flex; gap:1rem; flex-wrap:wrap;">
                    <div style="flex:1; min-width:200px;">
                        <label for="rating">Rating (1-5)</label>
                        <input type="number" id="rating" name="rating" min="1" max="5" value="{{ old('rating', $crop->rating) }}">
                    </div>
                    <div style="flex:1; min-width:200px;">
                        <label for="reviews">Reviews Count</label>
                        <input type="number" id="reviews" name="reviews" min="0" value="{{ old('reviews', $crop->reviews) }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        @php
                            $currentStatus = str_replace('-', '_', old('status', $crop->status));
                        @endphp
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" required>{{ old('description', $crop->description) }}</textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($crop->image_folder)
                        <img src="{{ asset('storage/' . $crop->image_folder) }}" alt="{{ $crop->name }}" class="current-image">
                    @endif
                    <div class="help">Upload to replace existing image. Leave blank to keep current.</div>
                </div>

                <div class="actions">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Update Crop</button>
