...
<head>
    ...
    <title>{{ $fertilizer->title }} - SpasilaLahanPetani</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="breadcrumb">
            ...
            <span>></span>
            <a href="{{ route('home') }}#fertilizers-section">Fertilizers</a>
            <span>></span>
            <span>{{ $fertilizer->title }}</span>
        </div>
            ...
            <div class="product-grid">
                ...
                @php
                    ...
                    if (!$primaryImage && $fertilizer->image) {
                        $primaryImage = strpos($fertilizer->image, 'images/fertilizer') !== false
                ...
                <div class="product-image">
                    @if(count($galleryUrls) > 1)
                        <button type="button" class="product-image-nav left" onclick="changeFertilizerImage('detail-image', -1)">
                    ...
                    @if(count($galleryUrls) > 1)
                        <button type="button" class="product-image-nav right" onclick="changeFertilizerImage('detail-image', 1)">
                ...
                <div class="product-details">
                    <h1>{{ $fertilizer->title }}</h1>

                    <div class="price">Rs {{ number_format($fertilizer->price, 2) }}</div>

                    <span class="status-badge {{ $fertilizer->status }}">
                    ...
                    <p class="description">{{ $fertilizer->description }}</p>

                    <div class="product-meta">
                        <div class="meta-item">
                            <label>Product ID</label>
                            <value>#{{ str_pad($fertilizer->id, 5, '0', STR_PAD_LEFT) }}</value>
                        </div>
                        <div class="meta-item">
                            <label>Status</label>
                            <value>{{ ucfirst(str_replace('_', ' ', $fertilizer->status)) }}</value>
                        </div>
                    </div>

                    <div class="action-buttons">
                        @if($fertilizer->status !== 'unavailable')
        ...
        <!-- Related Products -->
        @if($relatedFertilizers->count() > 0)
        <div class="related-products">
            <h2>Related Products</h2>
            <div class="products-grid">
                @foreach($relatedFertilizers as $relatedFertilizer)
                <a href="{{ route('fertilizers.show', $relatedFertilizer->id) }}" class="product-card">
                    <div class="product-card-image">
                        @if($relatedFertilizer->image)
                            @if(strpos($relatedFertilizer->image, 'images/fertilizer') !== false)
                                <img src="{{ asset($relatedFertilizer->image) }}" alt="{{ $relatedFertilizer->title }}">
                            @else
                                <img src="{{ asset('storage/' . $relatedFertilizer->image) }}" alt="{{ $relatedFertilizer->title }}">
                            @endif
                        @else
                    ...
                    <div class="product-card-body">
                        <div class="product-card-title">{{ $relatedFertilizer->title }}</div>
                        <div class="product-card-price">Rs {{ number_format($relatedFertilizer->price, 2) }}</div>
                    </div>
                </a>
                @endforeach
            </div>
